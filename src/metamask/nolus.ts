...
async function getAddress() {
  ...
  console.log("Nolus testnet address:", bech32Addr);
...
async function fetchAccountInfo(address: string) {
  try {
    ...
    if (!account) {
      console.log("Nolus account not found, may need funding");
    ...
    console.error("Error fetching Nolus account info:", error);
    throw error;
  }
}
...
async function broadcastTransaction(txBytes: Uint8Array) {
  try {
    ...
    if (result.error) {
      console.error("Nolus RPC Error:", result.error);
      throw new Error(`Nolus RPC Error: ${result.error.message}`);
    }

    return result.result;
  } catch (error) {
    console.error("Error broadcasting Nolus transaction:", error);
    throw error;
  }
}
...
export async function sendTransaction(toAddress?: string, amount?: string, memo?: string) {
  try {
    console.log("🚀 Sending Nolus Network transaction with MetaMask...");
    ...
    const exactSignDoc = {
      ...
      memo: memo || "Nolus Network transaction",
      msgs: [
        {
...
export async function checkTestnet() {
  try {
    console.log("🔍 Checking Nolus Network testnet...");
    ...
    const result = await response.json();
    console.log("Nolus testnet info:", result);
    return result;
  } catch (error) {
    console.error("Error checking Nolus testnet:", error);
    throw error;
  }
}
