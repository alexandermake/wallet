...
async function getAddress() {
  ...
  // Generate public key exactly like Secret.js does
  ...
  console.log("Secret testnet address:", bech32Addr);
...
async function fetchAccountInfo(address: string) {
  try {
    ...
    if (!account) {
      console.log("Secret account not found, may need funding");
    ...
    console.error("Error fetching Secret account info:", error);
    throw error;
  }
}
...
async function broadcastTransaction(txBytes: Uint8Array) {
  try {
    ...
    if (result.error) {
      console.error("Secret RPC Error:", result.error);
      throw new Error(`Secret RPC Error: ${result.error.message}`);
    }

    return result.result;
  } catch (error) {
    console.error("Error broadcasting Secret transaction:", error);
    throw error;
  }
}
...
export async function sendTransaction(toAddress?: string, amount?: string, memo?: string) {
  try {
    console.log("🚀 Sending Secret Network transaction with MetaMask...");
    ...
    const exactSignDoc = {
      ...
      memo: memo || "Secret Network transaction",
      msgs: [
        {
...
export async function checkTestnet() {
  try {
    console.log("🔍 Checking Secret Network testnet...");
    ...
    const result = await response.json();
    console.log("Secret testnet info:", result);
    return result;
  } catch (error) {
    console.error("Error checking Secret testnet:", error);
    throw error;
  }
}
